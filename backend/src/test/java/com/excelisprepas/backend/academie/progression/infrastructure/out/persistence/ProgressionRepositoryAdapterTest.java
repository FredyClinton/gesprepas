...
@Import({ProgressionRepositoryAdapter.class, ProgressionPersistenceMapper.class})
...
class ProgressionRepositoryAdapterTest extends AbstractIntegrationTest {
    ...
    void saveEtFindByIdRetrouveLaProgression() {
        ...
        assertThat(retrouve.get().getPhaseId()).isEqualTo(phaseId);
        assertThat(retrouve.get().getExercices()).contains("Exercices 1 à 5");
    }

    @Test
    @DisplayName("existsByFormationIdAndSessionIdAndPhaseIdAndMatiereIdAndSemaineAndNumeroCours détecte un doublon")
    void existsDetecteUnDoublon() {
        ...
        // When
        boolean existe = adapter.existsByFormationIdAndSessionIdAndPhaseIdAndMatiereIdAndSemaineAndNumeroCours(
                formationId, sessionId, phaseId, matiereId, 1, 1);
        boolean nExistePas = adapter.existsByFormationIdAndSessionIdAndPhaseIdAndMatiereIdAndSemaineAndNumeroCours(
                formationId, sessionId, phaseId, matiereId, 1, 2);
    ...
    void existsByFormationIdDetecteUneReference() {
        // Given
        UUID formationId = UUID.randomUUID();
        Progression progression = new Progression(UUID.randomUUID(), formationId, UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
    ...
    void findAllRetourneToutesLesProgressions() {
        adapter.save(new Progression(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
    ...
    void deleteByIdSupprimeLaProgression() {
        Progression progression = new Progression(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
                1, 1, "Thème", "Contenu", null);
        adapter.save(progression);
    ...
    void existsByMatiereIdDetecteUneReference() {
        UUID matiereId = UUID.randomUUID();
        adapter.save(new Progression(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), matiereId,
                1, 1, "Thème", "Contenu", null));
